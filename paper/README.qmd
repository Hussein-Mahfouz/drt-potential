...
Scenario 2 is meant to create DRT zones that complement FRT by only including the OD demand that is not well served by FRT, as explained in @sec-method-transport-supply. Scenario 3 is similar, but also excludes OD pairs that could be served by a better FRT service. This exclusion is based on criteria explained in @sec-method-transport-demand. The entire workflow is shown in @fig-flowchart-scenarios.

![Flowchart for determining input of different scenarios](images/flowchart_od_potential_demand.png){#fig-flowchart-scenarios}
...
To determine which OD pairs we want in our analysis ($OD_3$), we compare the potential demand on all routes

1.  For each OD

    1.  get potential demand on all routes directly serving it

    2.  IF potential demand on busiest direct route serving OD \> 50th percentile of route demand, we don't include it in $OD_{3}$
...
![Clustering OD pairs with poor PT supply (Scenario 2) - POLYGONS](images/map_clusters_scenario_2_equal_length_50000_bus_frac_grouped_gtfs_poly.png)
...
![Clustering results (Scenario 3)](images/map_clusters_scenario_3_equal_length_50000_bus_frac_grouped_gtfs.png)
...
![Clustering results (Scenario 3, OD pairs \< 10km)](images/map_clusters_scenario_3_equal_length_10000_bus_frac_grouped_gtfs.png){#fig-cluster-scenario-3-equal-bus-frac-length-10000}
