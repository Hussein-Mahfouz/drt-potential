...
This study aims to contribute to strategic level planning of DRT services by leveraging a spatial statistical approach that is able to identify clusters in spatial flow data [@tao2016spatial]. This approach is combined with an analysis of transport supply and demand in order to identify concentrated areas of travel demand that could be served by DRT. Transport demand is included so as not to propose DRT when there is potential for an improved FRT. To our knowledge, this is the first attempt at using spatial clustering of flow data in strategic DRT planning. We aim to answer questions: Can we identify spatial clustering patterns in commuting flows? What would these clusters look like if we exclude flows that can be served by a high frequency FRT? Do the clusters differ in the land use they cover, and in the number of flows they contain? Can these difference be used to inform the type of DRT service that could serve each cluster?
...
@fig-zones-one-map gives an overview of how the zones could be merged based on the needs of the different clusters. It becomes more clear that the zones are predominantly in the outskirts and in rural areas. Zones in urban areas, such as that in the South West covers an area with no public transport. Future work could explore where bus services could be added instead of taking the bus network as a fixed constraint [@pinto2020joint].
