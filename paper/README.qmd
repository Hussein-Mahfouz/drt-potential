...
Given these results, it has been suggested that DRT may only be useful[^3] in areas with poor PT level of service [@rath2023assessing]. Even so, in order to move towards the desired mode shift and reduce VKT, the service area needs to be informed by spatial and temporal demand in the region [@kagho2021demand].

[^3]: useful as in leads to more sustainable travel. Should I / how do I add equity here?
...
The main components of transport supply are the road network, and the public transport timetables for buses and rail. We get the updated road network from OSM. Bus timetable data is obtained in GTFS format from the Bus Open Data Service (BODS)[^4] and rail data is obtained from the Rail Delivery Group[^5].

[^4]: https://data.bus-data.dft.gov.uk/

[^5]: https://data.atoc.org/
