...
-   Methods for visualising OD flow data (based on geovizualization, grapph theory):

    -   "The visualization methods of OD flow data mainly include the flow map \[16--21\], the OD matrix, and the OD map \[22--25\]. Among them, the flow map better reflects the spatial characteristics of OD data.

-   Limitations of these methods:

    -   But there are also some problems in that method, such as the visual cluttering problem, the modifiable area unit problem, the normalization problem, the salience bias, and so on \[1,8,11\].

-   Methods to overcome limitations, but they lead to loss of spatial information, and difficulty in perceiving relationships between OD flows

    -   "In order to solve these problems, scholars have proposed edge rerouting \[26\], edge bundling \[27,28\], matrices of multiple maps \[24,29\] and other means to reduce clutter, but it does lead to the loss of spatial information of OD points, and the relationship between OD flows is difficult to be perceived."

<!--# from: Detecting arbitrarily shaped clusters in origin-destinatin flows using ant colony optimisation -->

-   Clustering has emerged as a method for spatial pattern identification in OD data. Clustering methods can be grouped into two groups:

    -   spatial statisitcs-based methods: these extend traditional spatial statistics (such as Moran's I, Getis-Ord G statistic, and Ripley's K) which aim to use significance tests of spatial homogeniety to identify anomalies in OD flows and the different scales at which clustering could exist. These methods however do not group the data into clusters

    -   heirarchical and density-based clustering methods to produce clusters.
