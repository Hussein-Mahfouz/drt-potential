...
Experts agree that DRT service provision should be based on demand [@dft2020regulatory]. However, there are no agreed upon methods for how demand gaps that suit DRT can be identified in a way that allows it to be well-integrated with public transport. In this section we go over a proposed approach to identify gaps in public transport provision and cluster demand from these gaps in order to inform potential DRT operating zones. First, we show how demand gaps can be identified through a supply and demand analysis; In @sec-method-transport-supply we show how public transport supply is calculated and define poor public transport supply, and in @sec-method-transport-demand, we explain potential demand and how it is calculated. In @sec-method-clustering-flow-data, we introduce a clustering approach for determining how travel demand is concentrated geographically. We apply the algorithm on two different datasets obtained using the methods in @sec-methods-preparing-data: 1) OD pairs with poor public transport supply, and 2) OD pairs with poor public transport supply and low potential demand . The entire workflow is presented in @fig-workflow. We present our case study city in @sec-case-study
...
![Urban Rural Classification - 1: most rural, 10: most urban](images/rural_urban.png){#fig-rural-urban alt="Urban Rural Classification - 1: most rural, 10: most urban" fig-align="center" width="75%"}
...
::: columns
::: {#tbl-od-scenarios .column width="90%"}
...
In this section we look at the clustering output of $OD_s$ and $OD_{sd}$. @fig-cluster-scenario-2 and @fig-cluster-scenario-3 presents the results for each respectively. IIn both of these plots, each facet in the plot represents one of the main clusters produced by the DBSCAN algorithm. The components of the visual are:
...
The results are similar to those of $OD_s$, but we see that the spatial extents of some clusters cover are larger area (this can be seen if you compare the first row in @fig-cluster-scenario-3 with that in @fig-cluster-scenario-2). $OD_{sd}$ has fewer origin-destination pairs (@tbl-od-scenarios) as we exclude all OD pairs that have sufficient demand to warrant an FRT service (see @sec-method-transport-demand). Our clustering algorithm finds clusters based on a minimum threshold of OD flows, and so fewer overall ODs means that a larger spatial coverage is required to obtain the same minimum threshold.

Interestingly, we see some new clusters appear. Cluster 11 and 12 connect the suburb of Morley and the village of Ardsley to the urban core. Many of the OD pairs in these clusters were added in the demand analysis (@sec-method-transport-demand), indicating that there is low potential demand on the routes that serve them. A DRT service could pool demand from these areas and connect them to a high frequency FRT. Some clusters also disappear: clusters 9 and 10 in @fig-cluster-scenario-2 do not appear in @fig-cluster-scenario-3. This is because Pudsey, Bramley, and the surrounding areas have sufficient demand to warrant an FRT service connecting them to the urban core, so the OD pairs that connext them to the urban core were removed in the demand analysis.
...
![Classification of areas covered by each cluster and the DRT zone extracted from it](images/figure_bar_urban_rural_compare_filter_no_filter_facet_cluster_scenario_3_length_50000.png){#fig-cluster-stats-1 width="80%"}

![Total commuters on the different OD pairs making up each cluster](images/figure_scatter_commuters_color_speed_facet_cluster_scenario_3_length_50000.png){#fig-cluster-stats-2 width="80%"}

![Distribution of commuters (weighted OD pairs) in each cluster. Commuters are faceted by the angle of their trajectory (x axis) and their commuting distance in km (y axis - right side) ](images/figure_bar_bearing_y_commuters_facet_grid_cluster_distance_3_length_50000.png){#fig-cluster-stats-3 width="80%"}
...
In this paper, we propose a method for delineating DRT operating zones. The method is meant to inform strategic DRT planning and ensure that it is well integrated with existing public transport services. Below we discuss the findings, assumptions, and limitations of our approach.

## Input data for clustering

In the approach outlined in this paper, we conduct a supply and demand analysis to ensure that we focus only on OD flows that are not, or do not have the necessary demand to be, served by high frequency FRT. We argue that this is an important step for insuring that DRT is part of an integrated public transport offering.

Our demand analysis is based on splitting OD demand equally between it's common lines. While this is justified by our need to represent route demand given hypothetical improvements in headway, more robust methods can be used for scenario modelling of such improvements. A strategic transport model could be used to better understand how ridership on different routes is impacted by headway improvements, and set optimal headways accordingly. OD demand could then be distributed between common lines in proportion to their headway. Given the complexity of such an approach, it was outside the scope of this work.
