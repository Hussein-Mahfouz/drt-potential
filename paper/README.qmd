...
Essential to our analysis is the use of DRT as part of an intermodal transport offering. It is clear that our clusters often overlap with high-frequency FRT [@fig-cluster-scenario-3], highlighting the potential for DRT to act as a feeder service. By limiting DRT coverage to areas that are not covered by a high-frequency FRT (step 5 in @fig-workflow), we aim to minimize unnecessary mode shift (e.g. FRT to DRT) and enhance the efficiency of public transport systems. Intermodal routing has only recently become possible in agent-based transport simulations [@haslebacher2018intermodal], with the integration of DRT into this intermodal framework being a subsequent development [@chouaki2023agent]. Such a framework should be used to test the feasibility of our DRT zones in facilitating intermodal trips, but it should be noted that the success of such integration is dependent on other factors besides operating zones, such as seamless transfers and fare structures that do not penalise the use of multiple modes.
