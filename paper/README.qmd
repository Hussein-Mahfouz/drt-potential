...
@fig-cluster-scenario-2 shows the clustering results for the $OD_{s}$ input. Each facet in the plot represents one of the main clusters produced by the DBSCAN algorithm. The components of the visual are:

-   OD pairs: these are the straight lines with different colors. The different colors represent the fraction of car to bus users, while the line thickness represents the number of commuters in that OD pair. The endpoints of the OD pairs are also shown, with size corresponding to the number of commuters.
...
Overall, we see that many of the clusters join up the urban core with its suburban / rural periphery. This is in line with research showing that DRT services can be used to efficiently compliment FRT systems that extend to rural areas [@sorensen2021much; @schluter2021impact]. As mentioned in @sec-intro, the ability of DRT schemes to concentrate demand in time and geography improves their likelihood of success.
...
![Distribution of commuters (weighted OD pairs) in each cluster. Commuters are faceted by the angle of their trajectory and their commuting distance](images/figure_bar_bearing_y_commuters_facet_grid_cluster_distance_3_length_50000.png){#fig-cluster-stats-3 width="80%"}
...
In our approach, we conduct a supply and demand analysis to ensure that we focus only on OD flows that are not, or do not have the necessary demand to be, served by high frequency FRT. We argue that this is an important step for insuring that DRT is part of an integrated public transport offering.
