...
-   they lack of integrity (consistency between sub-models). This means that they lack behavioral realism; allocation of individuals to destinations that are infeasible in reality as theydo not match their time budget
...
The second step is to attach an activity schedule to each individual in the synthetic population. This step requires a household travel survey (or time use dataset on a sample of the population). Each individual from the synthetic population is matched onto a profile from the household travel survey. Methods for matching include:
...
2.  data-driven methods
    ...
    1.  Sequence alignment https://doi.org/10.1111/j.1467-8306.2007.00536.x

    2.  Markov Chain Monte Carlo (MCMC) simulation https://www.sciencedirect.com/science/article/pii/S0968090X1630095X

    3.  Bayesian networks:

        1.  https://www.sciencedirect.com/science/article/pii/S0968090X20307105

        2.  https://www.sciencedirect.com/science/article/pii/S1877050923005707

        ```{=html}
        <!-- -->
        ```
        1.  Passed to activity based models and statistical models create activity chains step-by-step (CEMDAP; ActivitySim; actiTopp) - "More often, however, synthetic populations are passed to activity-based models (Arentze et al., 2007; Pendyala et al., 2012) where a sequence of statistical models is applied to construct activity chains step by step. Examples of activity-based models are CEMDAP (Bhat et al., 2008), which is based on the Dallas-Fort Worth region in the USA, or the rule-based model TASHA (Roorda et al., 2008), which is specifically designed for the Greater Toronto area in Canada. Other notable examples of activity-based models are ActivitySim (ActivitySim, 2020) and actiTopp (Hilgert et al., 2017)"
...
1.  AcBM (what lit review did I get this from?)
